...
interface SealantProductAutocompleteProps {
  brand: string;
  model: string;
...
export default function SealantProductAutocomplete({
  brand,
  model,
  onBrandChange,
  onModelChange,
  onProductSelect,
}: SealantProductAutocompleteProps) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      if (brand.length >= 1) {
        const results = await searchSealantBrands(brand);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      if (brand) {
        ...
        const results = await searchSealantModels(brand, model);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2 relative">
        <Label htmlFor="sealant-brand">Marka</Label>
      ...
      {/* Pole modelu */}
      <div className="space-y-2 relative">
        <Label htmlFor="sealant-model">Model / Nazwa</Label>
        <div className="relative">
          <Input
            id="sealant-model"
            ref={modelInputRef}
