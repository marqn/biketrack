...
interface BikeBrandModelFieldsProps {
  brand: string;
  model: string;
  onBrandChange: (brand: string) => void;
  onModelChange: (model: string) => void;
  onProductSelect: (product: BikeProduct | null) => void;
}

export default function BikeBrandModelFields({
  brand,
  model,
  onBrandChange,
  onModelChange,
  onProductSelect,
}: BikeBrandModelFieldsProps) {
  const [brandSuggestions, setBrandSuggestions] = useState<string[]>([]);
  const [modelSuggestions, setModelSuggestions] = useState<BikeProduct[]>([]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      if (brand.length >= 1) {
        const results = await searchBikeBrands(brand);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      if (brand) {
        ...
        const results = await searchBikeModels(brand, model);
        setModelSuggestions(results as unknown as BikeProduct[]);
          ...
          <Input
            ...
            onChange={(e) => {
              ...
              if (
                !newBrand
                  .toLowerCase()
                  .startsWith(lastBrandQuery.toLowerCase()) ||
          ...
          <Input
            ...
            onChange={(e) => {
              ...
              if (
                !newModel
                  .toLowerCase()
                  .startsWith(lastModelQuery.toLowerCase()) ||
